...
class TestPrinter(TestCase):
    ...
    def test_printer_speed(self):
        pages = 10
        expected = "Printed 10 pages in 5.000000 seconds."
        
        result = self.printer.print(pages)
        self.assertEqual(result,expected)
        
    def test_speed_always_two_decimals(self):
        fst_printer = Printer(pages_per_s=3.0,capacity=300)
        pages = 11
        expected = "Printed 11 pages in 3.666667 seconds."
        
        result = fst_printer.print(pages)
        self.assertEqual(result,expected)
